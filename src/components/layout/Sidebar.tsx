      ...
      <nav className="py-2 px-4">
        {menuGroups.map((group) => (
          <div key={group.id} className="mb-4">
            <button
              className="text-xs font-medium text-gray-400 mb-1 w-full text-left flex items-center justify-between"
              onClick={() => toggleGroup(group.id)}
            >
              {group.title}
              <span className="text-gray-300">{expandedGroups.includes(group.id) ? '▲' : '▼'}</span>
            </button>
