          ...
          <div key={member.id} className="bg-white rounded-2xl border border-gray-100 p-4 sm:p-5 flex items-start gap-3 sm:gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                ...
                <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold whitespace-nowrap" style={{ background: `${member.color}15`, color: member.color }}>
                  {member.role}
                </span>
              </div>
              <p className="text-[11px] text-gray-400 mb-3 truncate">{member.department}</p>
              <div className="space-y-1.5">
                {member.contact.map((c) => (
                  <div key={c.type} className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-500 truncate">{c.value}</span>
                  </div>
                ))}
              </div>
            </div>
