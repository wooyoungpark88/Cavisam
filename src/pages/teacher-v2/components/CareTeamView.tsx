...
// @ts-expect-error reserved for future filter UI
const _ROLE_FILTERS: RoleFilter[] = ["전체", "담임교사", "특수교사", "언어치료사", "작업치료사", "사회복지사", "행동지원전문가"];
...
function AssignmentsTab({ selectedMemberId }: { selectedMemberId: number | null }) {
  return (
    <div className="p-5 space-y-3">
      <div className="flex items-center justify-between mb-1">
        <p className="text-xs font-bold text-gray-500 uppercase tracking-wide">이용인 담당 현황</p>
        <p className="text-[11px] text-gray-400">총 7명</p>
      </div>
      <div className="space-y-2">
        {mockStudentAssignments.map((sa) => {
            ...
            >
              ...
              <div className="flex flex-wrap gap-1.5">
                {sa.assignedMemberIds.map((mid) => {
                  const m = mockTeamMembers.find((t) => t.id === mid);
                  if (!m) return null;
...
export default function CareTeamView() {
  ...
  const filteredMembers =
    roleFilter === "전체"
      ? mockTeamMembers
      : mockTeamMembers.filter((m) => m.role === roleFilter);
  ...
  return (
    <div className="flex h-full overflow-hidden">
      {/* ── Left: Team list ── */}
      <aside className="flex flex-col border-r border-gray-100 bg-white overflow-hidden" style={{ width: 256 }}>
        {/* Header */}
        <div className="flex-shrink-0 px-4 pt-5 pb-3 border-b border-gray-100">
          <div className="flex items-center justify-between mb-1">
            <h2 className="text-gray-900 text-sm font-bold">돌봄 팀</h2>
            <span className="text-[11px] text-gray-400">{mockTeamMembers.length}명</span>
          </div>
        ...
        <div className="flex-shrink-0 px-3 py-2.5 border-b border-gray-100">
          <div className="flex flex-wrap gap-1">
            {["전체", "교사", "치료사", "복지사", "전문가"].map((label) => {
              ...
              const isActive = roleFilter === filterVal ||
        ...
        {selectedMember && (
          <div className="flex-shrink-0 border-t border-gray-100 px-4 py-4 space-y-2.5">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-start gap-2">
                <div className="w-3.5 h-3.5 flex items-center justify-center flex-shrink-0 mt-0.5">
                  <i className="ri-mail-line text-[10px] text-gray-400" />
                </div>
              ...
              <div className="flex items-start gap-2">
                <div className="w-3.5 h-3.5 flex items-center justify-center flex-shrink-0 mt-0.5">
                  <i className="ri-star-line text-[10px] text-gray-400" />
                </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Top info bar */}
        <div className="flex-shrink-0 px-5 py-3.5 border-b border-gray-100 bg-white flex items-center justify-between">
              ...
              <>
                ...
                <div>
                  ...
                  <p className="text-gray-400 text-[10px]">
                    담당 이용인 {selectedMember.assignedStudentIds.length}명 · {selectedMember.department}
                  </p>
                </div>
                <button
                  onClick={() => setSelectedMemberId(null)}
                  className="ml-2 text-[10px] text-gray-400 hover:text-gray-600 cursor-pointer whitespace-nowrap"
                >
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-3 text-[11px] text-gray-500">
              {[
                { label: "긴급", color: "#dc2626", count: mockStudentAssignments.filter(s => s.priority === "긴급").length },
                { label: "집중", color: "#d97706", count: mockStudentAssignments.filter(s => s.priority === "집중").length },
              ].map((item) => (
        ...
        {/* Tabs */}
        <div className="flex-shrink-0 flex items-center gap-1 px-5 pt-3 pb-0 border-b border-gray-100 bg-white">
          {TABS.map((tab) => {
            ...
            return (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold cursor-pointer whitespace-nowrap transition-all border-b-2"
                style={{
                  color: isActive ? "#026eff" : "#6b7280",
        ...
        <div className="flex-1 overflow-y-auto bg-gray-50/50">
          {activeTab === "assignments" && <AssignmentsTab selectedMemberId={selectedMemberId} />}
